...
## vLLM 蒸留（Distillation）パイプライン

Teacher vLLM の推論結果を Logstash TCP :5000 経由で Elasticsearch データストリーム `logs-vllm-distill` に取り込みます。

### デプロイ

```bash
# ELK 側（テンプレート/ILM + Logstash フィルタ + NetworkPolicy）
kubectl apply -k elk-stack/overlays/kubeadm/      # 本番 ILM 30 日
kubectl apply -k elk-stack/overlays/kind/         # kind ILM 7 日
