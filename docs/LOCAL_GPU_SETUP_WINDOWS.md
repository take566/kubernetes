...
| 経路 | GPU | 用途 |
|------|-----|------|
| **A: Windows Ollama → K8s 外部 Service** | AMD RX 5700（DirectML/Windows ドライバ） | **推奨** — クラスタ内 Pod から `ollama-external.vllm` で呼び出し |
...
| Ollama が CPU のみ | タスクマネージャで GPU 使用率を確認。ログに `AMD driver is too old` がある場合は [AMD Adrenalin](https://www.amd.com/en/support) を更新し `restart-ollama-wsl-bridge.ps1` |
