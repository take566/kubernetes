...
if kubectl -n ingress-test wait --for=condition=Ready pod/ingress-verify-curl --timeout=60s 2>/dev/null; then
  :
fi
kubectl -n ingress-test wait --for=jsonpath='{.status.phase}'=Succeeded pod/ingress-verify-curl --timeout=90s
