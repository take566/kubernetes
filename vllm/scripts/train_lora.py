...
Optimized defaults for AMD Instinct / ROCm PyTorch:
- bf16 training (preferred over fp16 on MI200/MI300)
- SDPA attention (ROCm-friendly; avoids flash-attn build complexity)
...
def main() -> int:
    # Lazy imports so dependency install can happen in the Job entrypoint first.
    import torch
    ...
    text_field = os.getenv("DATASET_TEXT_FIELD", "text")
...
def _load_dataset(dataset_path: str):
    ...
    # Directory: HuggingFace dataset layout or single train.jsonl
