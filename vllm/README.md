...
| **kind**（ローカル） | `vllm/overlays/kind` | `vllm-kind`（別途追加予定） |
| minikube | `vllm/`（hostPath） | **非推奨** — [deprecated/minikube/](../deprecated/minikube/README.md) |

ルート `vllm/` は Kustomize **base**（hostPath PV）です。本番・Argo CD では必ず **overlay** を使ってください。
...
> **本番 / kubeadm:** 上記 overlay を使用。以下の `kubectl apply -k vllm/` は minikube 向け legacy です。
...
### 推論: NVIDIA（legacy base / minikube — 非推奨）
...
# legacy hostPath（minikube — 非推奨）:
# ノード上: mkdir -p /data/vllm/finetune/{dataset,output,huggingface}
...
├── kustomization.yaml            # NVIDIA base（hostPath — overlay 必須）
├── overlays/
